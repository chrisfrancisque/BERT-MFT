"""
Working MFT implementation with proper gradient flow
"""
import torch
...
class MFTHandler(nn.Module):
    """MFT handler that properly maintains gradient flow"""
    ...
    def _initialize_scores(self):
        ...
        print("Initializing learnable mask scores...")
        total_params = 0
        maskable_params = 0
    ...
    def _should_mask(self, name):
        ...
        # Must be attention or FFN weight
    ...
    def get_masks(self, masking_ratio=0.1):
        ...
        for safe_name, scores in self.mask_scores.items():
            ...
            # Create mask keeping top (1-masking_ratio) percent
...
def train_mft(model, train_dataloader, eval_dataloader, config, epochs=2, lr=5e-4):
    """Train MFT masks"""
    # Create MFT handler
    ...
    for epoch in range(epochs):
        ...
        for batch in progress_bar:
            ...
            for scores in mft_handler.mask_scores.values():
                l1_loss += 0.001 * torch.abs(scores).sum()
            loss = loss + l1_loss
    ...
    if best_masks:
        apply_masks_permanently(model, best_masks)
    
    return best_accuracy


def apply_masks_permanently(model, masks):
    """Apply masks permanently to model"""
    print("\nApplying masks permanently...")
    total_masked = 0
    total_params = 0
