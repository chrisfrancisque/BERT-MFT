...
class MFTHandler:
    ...
    def __init__(self, model, config):
        ...
        self.param_name_mapping = {}  # Map sanitized names to original names
    ...
    def _sanitize_param_name(self, name):
        """Convert parameter name to valid ParameterDict key"""
    ...
    def _initialize_scores(self):
        ...
        for name, param in self.model.named_parameters():
            ...
            if self._should_mask(name):
                # Sanitize the name for ParameterDict
                sanitized_name = self._sanitize_param_name(name)
                
                # Store mapping
                self.param_name_mapping[sanitized_name] = name
                
                # Initialize scores with small random values
                self.scores[sanitized_name] = nn.Parameter(
        ...
        print(f"  Number of parameter groups to mask: {len(self.scores)}")
    
    def create_masks(self, masking_ratio=0.9):
        """Create binary masks from learned scores"""
        masks = {}
        
        for sanitized_name, scores in self.scores.items():
            # Get original parameter name
            original_name = self.param_name_mapping[sanitized_name]
            
            if masking_ratio == 1.0:
                # Use threshold-based masking
                masks[original_name] = StraightThroughEstimator.apply(scores)
            else:
                ...
                mask[topk_idx] = 1.0
                masks[original_name] = mask.reshape(scores.shape)
    ...
    def restore_params(self, original_params):
        ...
        with torch.no_grad():
            for name, param_data in original_params.items():
                # Navigate to the parameter using the dot notation
                module = self.model
                ...
                # Set the parameter
                setattr(module, name_parts[-1], nn.Parameter(param_data))
        ...
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                if name in masks:
                    ...
                    if len(self.scores) < 20 or 'layer.4' in name:
